module NSWTopo
  class FeatureSource
    ...
    # For Shapefile formated sources
    ...
    # For json and other Shapeiles (victopo version)
    
    def shp_features(map, source, options)
      path = options["path"]
      type_name = options["name"]
      ...
      mapprojedction = map.projection
      bounds = map.bounds
      spat = bounds.flatten.values_at(0,2,1,3).join ?\s
      
      jsonpath = Tempfile.new(['out', '.json']).path

      %x[ogr2ogr -f "GeoJSON" -spat #{spat} -where "#{filter}" -spat_srs "#{mapprojedction}" -t_srs "#{mapprojedction}" -nln "#{type_name}" #{jsonpath} #{path}]
    
      parsedjson = JSON.parse((File.read(jsonpath)))
          
      Enumerator.new do |yielder|
          parsedjson.fetch("features").each do |feature|
            geometry = feature["geometry"]
            dimension = case geometry["type"]
            when "Polygon", "MultiPolygon" then 2
            when "LineString", "MultiLineString" then 1
        ...
        options_array.inject([]) do |memo, options|
          ...
          fallbacks.inject(nil) do |error, options|
            ...
            begin
              ...
              when "shp"       then        shp_features(map, source, options)
